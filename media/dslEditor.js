"use strict";
(() => {
  // media/src/snippetEngine.ts
  var SnippetEngine = class {
    constructor() {
      this.mode = false;
      this.tabStopWords = [];
    }
    ...
    activate(edit) {
      this.mode = edit.tabStops !== void 0 && edit.tabStops.length >= 1;
      if (!this.mode) {
        this.tabStopWords = [];
      }
    }
    setTabStopWords(words) {
      this.tabStopWords = words;
    }
    tabStopRegex(global) {
      const words = this.tabStopWords.length > 0 ? this.tabStopWords.map((w) => w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|") : "NAME|name|type|value|values";
      return new RegExp(`\\b(${words})\\b`, global ? "g" : "");
    }
    extractTabStopWords(text) {
    ...
    advanceToNextTabStop(source2) {
      if (!this.mode) {
        return false;
      }
      const cursor = source2.selectionEnd;
      const text = source2.value;
      const lineEnd = text.indexOf("\n", cursor);
      const searchEnd = lineEnd === -1 ? text.length : lineEnd;
      const match = this.tabStopRegex().exec(text.slice(cursor, searchEnd));
      if (match !== null) {
        source2.selectionStart = cursor + match.index;
        source2.selectionEnd = cursor + match.index + match[0].length;
        return true;
      }
      this.mode = false;
      this.tabStopWords = [];
    ...
    highlightTabStops(lines, cursorEnd, value, highlightLine2, escapeHtml2) {
      ...
      if (tabStopPositions.length === 0) {
        return lines.map(highlightLine2).join("\n");
      }
      let offset = 0;
      return lines.map((line) => {
        ...
        for (const ts of lineTabStops) {
          ...
          result += highlightLine2(line.slice(pos, start));
          result += `<span class="tab-stop">${escapeHtml2(line.slice(start, end))}</span>`;
          pos = end;
        }
        return result + highlightLine2(line.slice(pos));
      }).join("\n");
    }
  };

  // media/src/dslEditor.ts
  ...
  var suggestionInsertText = "";
  var snippetEngine = new SnippetEngine();
  var navHoverRange;
  var completionCandidates = [];
  var completionInsertTexts = [];
  ...
  function setDiagnosticLines(lines) {
    diagnosticLines = Array.isArray(lines) ? [...new Set(lines.filter((line) => Number.isInteger(line) && line > 0))].sort((left, right) => left - right) : [];
    renderErrorLines();
  }
  ...
  function renderCompletionList() {
    completionList.innerHTML = completionCandidates.map((label, i) => {
      ...
      return `<div class="completion-item${i === completionIndex ? " active" : ""}" data-index="${i}"><span class="completion-icon-col ci-${colorClass}"><i class="codicon codicon-${iconClass}" aria-hidden="true"></i></span><span class="completion-label">${escapeHtml(label.includes(" ") ? label.replace(/:$/, "") : label)}</span></div>`;
    }).join("");
  }
  ...
  function paintHighlight() {
    ...
    const highlighted = navHoverRange ? highlightNavigationRange(lines) : hasGhost ? highlightLinesWithGhost(lines) : snippetEngine.active ? snippetEngine.highlightTabStops(lines, source.selectionEnd, source.value, highlightLine, escapeHtml) : lines.map(highlightLine).join("\n");
  ...
  function acceptSuggestion() {
    ...
    replaceSelection(edit.text, void 0, edit.selection);
    clearSuggestion();
  ...
  function goToDeclaration() {
    ...
    const position = tokenInfo ? findDeclaration(tokenInfo.token) : void 0;
    if (position === void 0) {
      return false;
    }
  ...
  function updateNavigationHover(event) {
    ...
    if (!isNavigation) {
      navHoverRange = void 0;
    } else {
      const position = getPositionFromPointer(event);
      const tokenInfo = position === void 0 ? void 0 : getTokenAtPosition(position);
      navHoverRange = tokenInfo && findDeclaration(tokenInfo.token) !== void 0 ? { start: tokenInfo.start, end: tokenInfo.end } : void 0;
    }
    source.closest(".editor").classList.toggle("has-nav-hover", navHoverRange !== void 0);
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.type === "error") {
      ...
      if (diagnosticLines.length > 0) {
        ...
        const offset = source.value.split("\n").slice(0, firstLine - 1).reduce((sum, line) => sum + line.length + 1, 0);
